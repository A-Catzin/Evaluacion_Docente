        ...
        try {
          ...
          if (plan) {
            ...
            try {
              ...
              console.error('[Coordinador Planeación] Error notificando docente:', err);
            }
            try {
              await recalcularCalificacionDocente(cl, Number(plan.docente_id), Number(plan.cuatrimestre_id));
            } catch (recalcError) {
              logRecalcError(Number(plan.docente_id), Number(plan.cuatrimestre_id), recalcError);
            }
          }
        } catch (err) {
