...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    const { data: usuario } = await cliente
      .from("usuarios")
      .select("entidad_id,rol,email")
      .eq("id", userId)
      .maybeSingle();
    if (!usuario) {
      return new Response(
        JSON.stringify({
          error: "Solo docentes pueden enviar autodiagnóstico",
        }),
        { status: 403 },
      );
    }
    ...
    if (!parseResult.success) {
      return new Response(
        JSON.stringify({
          error: "Todos los campos son obligatorios excepto comentarios",
    ...
    if (docenteId) {
      const { error: errUpd } = await cliente
        ...
        .eq("id", docenteId);
      if (errUpd)
        throw new Error("Error al actualizar docente: " + errUpd.message);
    } else {
      const { data: existenteDoc } = await cliente
        .from("docentes")
        .select("id")
      ...
      if (existenteDoc) {
        ...
        const { data: nuevo, error: errIns } = await cliente
          .from("docentes")
          .insert({
            nombre,
            apellido_paterno,
          ...
          .single();
        if (errIns)
          throw new Error("Error al crear docente: " + errIns.message);
        docenteId = nuevo.id;
        await cliente
          .from("usuarios")
          .update({ entidad_id: docenteId })
    ...
    const { data: existente } = await cliente
      ...
      .eq("cuatrimestre_id", cuatrimestre_id)
      .maybeSingle();
    if (existente) {
      return new Response(
        JSON.stringify({
          error: "Ya completaste tu autodiagnóstico para este cuatrimestre",
        }),
        { status: 409 },
      );
    }
    ...
    if (moderacion.valorNormalizado)
      insert.comentarios = moderacion.valorNormalizado;

    const { data: resultado, error: errDiag } = await cliente
      .from("autodiagnosticos")
      .insert(insert)
      .select("puntaje_total,nivel_desempeno")
      .single();
    if (errDiag) {
    ...
    try {
      await recalcularCalificacionDocente(cliente, docenteId, cuatrimestre_id);
    } catch (recalcError) {
      logRecalcError(docenteId, cuatrimestre_id, recalcError);
    }
