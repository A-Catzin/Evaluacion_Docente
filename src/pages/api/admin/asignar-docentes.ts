...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    const { evaluador_id, docente_ids } = body;
    ...
    await cl.from('coordinador_docentes').delete().eq('coordinador_id', evaluador_id);

    // Insertar nuevas
    if (docente_ids.length > 0) {
      const inserts = docente_ids.map(did => ({ coordinador_id: evaluador_id, docente_id: did }));
