...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    const { action, instrumento, texto, orden, id, activa } = body;

    if (action === 'create') {
      const { error } = await cl.from('instrumento_preguntas').insert({ instrumento, texto, orden: orden || 1 });
      if (error) return new Response(JSON.stringify({ error: error.message }), { status: 400 });
      return new Response(JSON.stringify({ success: true }), { status: 201 });
    }
    if (action === 'update') {
      const { error } = await cl.from('instrumento_preguntas').update({ texto }).eq('id', id);
      if (error) return new Response(JSON.stringify({ error: error.message }), { status: 400 });
