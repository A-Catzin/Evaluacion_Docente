...
export type PlanningSubmissionMode = "manual_open" | "manual_closed" | "scheduled";
export type PlanningSubmissionState = "open" | "closed" | "not_configured" | "scheduled_pending" | "scheduled_ended";
...
export function planningWindowFromRpc(data: unknown): PlanningWindow {
  ...
  if (!row || typeof row !== "object") {
    return { configured: false, mode: null, opens_at: null, closes_at: null, state: "not_configured" };
  }
  const value = row as Record<string, unknown>;
  const state = value.state;
  const mode = value.mode;
  if (!["open", "closed", "not_configured", "scheduled_pending", "scheduled_ended"].includes(String(state))) {
    return { configured: false, mode: null, opens_at: null, closes_at: null, state: "not_configured" };
  }
  return {
    configured: value.configured === true,
    mode: ["manual_open", "manual_closed", "scheduled"].includes(String(mode)) ? mode as PlanningSubmissionMode : null,
...
export async function getPlanningSubmissionWindow(client: SupabaseClient, cuatrimestreId: number): Promise<PlanningWindow> {
  if (!Number.isSafeInteger(cuatrimestreId) || cuatrimestreId <= 0) {
    return { configured: false, mode: null, opens_at: null, closes_at: null, state: "not_configured" };
  }
  const { data, error } = await client.rpc("planning_submission_window_state", { p_cuatrimestre_id: cuatrimestreId });
  if (error) return { configured: false, mode: null, opens_at: null, closes_at: null, state: "not_configured" };
  return planningWindowFromRpc(data);
}

export async function requireTeacherPlanningSubmissionOpen(client: SupabaseClient, cuatrimestreId: number): Promise<Response | null> {
  const window = await getPlanningSubmissionWindow(client, cuatrimestreId);
  if (window.state === "open") return null;
  return new Response(JSON.stringify({ error: planningSubmissionMessage(window), code: "planning_submissions_closed" }), {
    status: 403,
...
export function validatePlanningPdf(file: File): PlanningPdfValidation {
  ...
  if (file.type !== "application/pdf" || !file.name.toLowerCase().endsWith(".pdf")) {
    return { ok: false, error: "Solo se aceptan archivos PDF con extensión .pdf." };
  }
  return { ok: true, extension: "pdf" };
}

export function parsePositiveInteger(value: FormDataEntryValue | unknown): number | null {
...
export async function resolveTeacherPlanningGroup(
  ...
  const { data, error } = await client
    ...
    .eq("activo", true)
    .maybeSingle();
  if (error || !data || data.modalidad !== "Escolarizada") return null;
  return data;
}

export function buildPlanningPdfPath(cuatrimestreId: number, docenteId: number): string {
...
export function parseCancunDateTimeLocal(value: unknown): Date | null {
  ...
  return `${parts.year}-${parts.month}-${parts.day}T${parts.hour}:${parts.minute}` === value ? parsed : null;
}

export function formatCancunDateTimeLocal(value: string | null | undefined): string {
  if (!value) return "";
  const parts = new Intl.DateTimeFormat("en-CA", {
    timeZone: PLANNING_TIME_ZONE,
    year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hourCycle: "h23",
...
export function parsePlanningWindowInput(body: Record<string, unknown>) {
  ...
  if (!cuatrimestreId || !["manual_open", "manual_closed", "scheduled"].includes(String(mode))) {
    return { ok: false as const, error: "La configuración de acceso no es válida." };
  }
  if (mode !== "scheduled") {
    return { ok: true as const, value: { cuatrimestreId, mode: mode as PlanningSubmissionMode, opensAt: null, closesAt: null } };
  }
  const opensAt = parseCancunDateTimeLocal(body.opens_at);
  const closesAt = parseCancunDateTimeLocal(body.closes_at);
  if (!opensAt || !closesAt || opensAt >= closesAt) {
    return { ok: false as const, error: "La ventana programada requiere fechas y horas válidas en horario de Cancún, con inicio anterior al cierre." };
  }
  return { ok: true as const, value: { cuatrimestreId, mode, opensAt: opensAt.toISOString(), closesAt: closesAt.toISOString() } };
}
